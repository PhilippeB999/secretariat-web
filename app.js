...
const STORAGE_KEY = "secretariatquest_state_v1";
...
const ACCESS_CODES = ["SECRETARIAT-2026-3GMW"]; // code(s) fourni(s) au centre de formation après achat de la licence
...
const SYNC_KEY = "secretariatquest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("secretariatquest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("secretariatquest_device_id", id);
  }
  return id;
}
